...
class index extends Controller
	{
		...
		public function topup()
		{
			...
			$topupinfo=array(
				'TopupBusinessId'=>$_POST['udf2'],
				'TopupPeopleId'=>$_POST['udf1'],
				'Amt'=>$_POST['amount'],
				'Status'=>'1'
			);
			$this->index_model->insert('topups',$topupinfo);
			if($_POST['udf3'])
			{	
				$walletinfo=array(
			...
			$this->mailer->username("topup@my-deal.in");
			$this->mailer->password("1_topup_1");
			$this->mailer->from("topup@my-deal.in");
			$this->mailer->sender("MY DEAL");
			$this->mailer->to($payu['email']);
			$this->mailer->subject("Topup Successful-MY DEAL");
			$this->mailer->message($payu['firstname'].' '.$payu['lastname'].' '."you have Topup succesfully");
			$this->mailer->send();
			$this->payu($_POST);
			$info['val']=1;
